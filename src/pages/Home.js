...
function addMessage(newMessage) {
    return (previousState, currentProps) => {
    	console.log("____________ changed messages");
       return { ...previousState, messages: previousState.messages.concat(newMessage) };
    };
}
...
export default class Home extends React.Component{
	...
	constructor(){
		...
		this.sampleCommands = ["Move R on top of A", "why move N to Y?", "How did you do that?", "how did you clear the top of A?","Pls move box Y ontop of N." ];
		this.sampleIndex = 0;
		this.state={
			context: {
				command:"",
	...
	showBoxes(){
		...
		proccess(){
		//this.setState({messages:this.state.messages.concat(new MessageClass(this.state.command, "user", 0))});
		if(this.state.command==""){
			return;
		}
		this.sampleIndex+=1;
		this.setState({command:this.sampleCommands[this.sampleIndex]});
		this.setState(addMessage(new MessageClass(this.state.command, "user", 0)));
		const command=this.state.command.replace(/[^a-zA-Z ]/g, "");
		const part = command.split(" ");
		let simple = part.map((word)=>{
			if(word.length>1){
		...
		if(simple.length==0||!(simple[0]=='reset'||simple[0]=='why'||simple[0]=='how'||simple[0]=='move'||simple[0]=='help')){
			this.setState(addMessage(new MessageClass("sorry wat?", "me", 2*Math.random())));
			return;
		}else if(simple.find((word)=>{return word=='that';})!==undefined && this.state.context.command==""){
			this.setState(addMessage(new MessageClass("sorry what do you mean by 'that'? :p", "me", 2*Math.random())));
			return;
			...
			this.setState(addMessage(new MessageClass(manual , "me", 0)));
			return;
		}
		console.log(simple);
		let first_word=simple[0];
		A_ID = simple.find((word)=>{return word.length==1;});
		B_ID = simple.slice(simple.indexOf(A_ID)+1).find((word)=>{return word.length==1;});
		
		if(simple.find((word)=>{return word=='that';})!==undefined && this.state.context.command!==""){
			console.log("editting")
			const question = simple.find((word)=>{return word=='move'||word=='clear';});
			if (question==undefined){
				simple.splice(1,1,this.state.context.command);
			}
			if (A_ID==undefined){
				A_ID=this.state.context.A;
				simple=[...simple,A_ID];
			}
			if (B_ID==undefined){
				B_ID=this.state.context.B;
				simple=[...simple,B_ID];
			}
		}
			console.log(simple);
		...
		if(first_word=='move'){
			...
			if(A_ID==undefined||B_ID==undefined){
				this.setState(addMessage(new MessageClass("move what on top of where?", "me", 2*Math.random())));
				return;
			}
				const responses = ['ok.','fine.','sure thing boss!','whatever...','lol sure','kk', ' so bossy -_-'];
				let index =Math.floor(Math.random()*(responses.length));
				const reponse = responses[index];
				this.setState(addMessage(new MessageClass(reponse, "me", 0)));
				this.setState({A:A_ID, B:B_ID });
		...
		findHow(question){
			if(question==null){
				...
				if(task==null){
					...
					this.setState(addMessage(new MessageClass("hmmm did I do that?", "me", 2*Math.random())));
				}
				else if(children.length==0){

					this.setState(addMessage(new MessageClass("just kinda did it xD", "me", 2*Math.random())));
				}else {
					...
					this.setState(addMessage(new MessageClass("I "+how+ end, "me", 2*Math.random())));
				}
			}
			var self=this;
			setTimeout(function() {
		...
		findWhy(question){
			if(question==null){
				...
				if(parent==null){
					...
					this.setState(addMessage(new MessageClass("cus u told me to.", "me", 2*Math.random())));
				}else {

					this.setState(addMessage(new MessageClass("to "+parent.value, "me", 2*Math.random())));
				}
			}
			var self=this;
			setTimeout(function() {
		...
		if(this.above(A, A.width).size!==0){
			...
			this.clearTop(A_ID, B_ID, A_ID, B_ID, A.width);
		}else if(this.above(B, A.width).size!==0){
			this.addGoal("clear the top of "+B_ID);
			this.clearTop(B_ID, A_ID, A_ID, B_ID, A.width);
		}else{
...
chatIntro(delay){
	...
	const self = this;
	setTimeout(function() {
		self.setState(addMessage(new MessageClass(hi , "me", 0)));
	}, delay);
	setTimeout(function() {
		self.setState(addMessage(new MessageClass(manual , "me", 0)));
	}, delay+3000);
}
...
componentWillUnmount(){
	...
	render(){
		return(
			...
			<div class="chatInput">
			<Messenger id="messages" messages={this.state.messages}/>
